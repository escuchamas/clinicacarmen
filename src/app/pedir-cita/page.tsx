...
type Step = "identificar" | "gestionar" | "fecha" | "hora" | "confirmar" | "exito" | "cancelada";
...
export default function PedirCitaPage() {
  ...
  async function handleIdentificar(e: React.FormEvent) {
    ...
    setLoadingId(false);
    if (!res.ok) { setErrorId(data.error); return; }
    setPaciente(data);
    if (data.citaActiva) {
  ...
  return (
    <div style={{ fontFamily: "system-ui, -apple-system, sans-serif", color: DARK, minHeight: "100vh", backgroundColor: CREAM }}>
      ...
      <main style={{ maxWidth: 560, margin: "0 auto", padding: "2.5rem 1.5rem" }}>
        ...
        {/* Progreso */}
        {step !== "exito" && step !== "gestionar" && step !== "cancelada" && (
          <div style={{ marginBottom: "2rem" }}>
            <div style={{ display: "flex", gap: "0.5rem", marginBottom: "0.75rem" }}>
              {[1, 2, 3, 4].map(n => (
                <div key={n} style={{ flex: 1, height: 4, borderRadius: 999, backgroundColor: n <= stepNum ? AQUA : "#e5e7eb", transition: "background 0.3s" }} />
              ))}
            </div>
            <p style={{ fontSize: "0.8125rem", color: "#6b7280" }}>
              Paso {stepNum} de 4 · {["Identificación", "Elige fecha", "Elige hora", "Confirmar y pagar"][stepNum - 1]}
            </p>
          </div>
        )}
        ...
        {step === "confirmar" && (
          <div>
            ...
            <h2 style={{ fontSize: "1.375rem", fontWeight: 900, letterSpacing: "-0.02em", marginBottom: "1.75rem" }}>Confirmar y pagar</h2>
            ...
            <div style={{ backgroundColor: "white", borderRadius: "1.25rem", padding: "1.5rem", border: "1px solid #e5e7eb", marginBottom: "1rem" }}>
              ...
              <div style={{ display: "grid", gap: "0.75rem" }}>
                <ResumenItem icon={<User size={15} color={AQUA} />} label="Paciente" value={`${paciente?.nombre} ${paciente?.apellidos}`} />
                <ResumenItem icon={<Calendar size={15} color={AQUA} />} label="Fecha" value={fechaLabel} />
                <ResumenItem icon={<Clock size={15} color={AQUA} />} label="Hora" value={`${horaSeleccionada} — ${horaFin(horaSeleccionada)}`} />
              </div>
            ...
            <div style={{ backgroundColor: "white", borderRadius: "1.25rem", padding: "1.5rem", border: "1px solid #e5e7eb", marginBottom: "1rem" }}>
              ...
              <div style={{ display: "grid", gap: "0.875rem", opacity: 0.45, pointerEvents: "none" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
                  <div>
                    <label style={labelSt}>Caducidad</label>
                    <div style={{ ...inputSt, color: "#9ca3af" }}>MM / AA</div>
                  </div>
                  <div>
                    <label style={labelSt}>CVC</label>
                    <div style={{ ...inputSt, color: "#9ca3af" }}>···</div>
                  </div>
                </div>
              </div>
              ...
              <div style={{ marginTop: "1rem", padding: "0.75rem 1rem", backgroundColor: "#FEF3C7", borderRadius: "0.625rem", fontSize: "0.8125rem", color: "#92400E", display: "flex", gap: "0.5rem" }}>
                <span>⚙️</span>
                <span>El sistema de pago se activará próximamente. Tu cita quedará registrada como <strong>pendiente de pago</strong> y Carmen te confirmará.</span>
              </div>
            </div>
